...
VSIX_FILE = extension-performance-monitor-$(VERSION).vsix
...
clean-vsix:
	...
	@rm -f ./extension-performance-monitor-*.vsix
...
package:
	...
	@vsce package
	@rm -f ./extension-performance-monitor-latest.vsix
	@cp ./$(VSIX_FILE) ./extension-performance-monitor-latest.vsix

# Verify package includes node_modules
verify-package:
	@echo "Verifying package contents..."
	@VSIX_SIZE=$$(stat -c%s ./extension-performance-monitor-latest.vsix 2>/dev/null || stat -f%z ./extension-performance-monitor-latest.vsix); \
	if [ $$VSIX_SIZE -lt 100000 ]; then \
		echo "ERROR: VSIX is too small ($$VSIX_SIZE bytes). node_modules likely missing!"; \
		echo "Expected size > 100KB for extension with dependencies."; \
		exit 1; \
	fi; \
	echo "Package size OK: $$VSIX_SIZE bytes"
	@unzip -l ./extension-performance-monitor-latest.vsix | grep -q "node_modules/pidusage" || (echo "ERROR: pidusage not found in VSIX!" && exit 1)
	@unzip -l ./extension-performance-monitor-latest.vsix | grep -q "node_modules/ps-tree" || (echo "ERROR: ps-tree not found in VSIX!" && exit 1)
...
install-all:
	...
	@if command -v code >/dev/null 2>&1; then \
		...
		code --install-extension ./extension-performance-monitor-latest.vsix --force 2>/dev/null || true; \
	fi
	@if command -v code-insiders >/dev/null 2>&1; then \
		echo "Installing to VS Code Insiders..."; \
		code-insiders --install-extension ./extension-performance-monitor-latest.vsix --force 2>/dev/null || true; \
	fi
	@if command -v windsurf >/dev/null 2>&1; then \
		echo "Installing to Windsurf..."; \
		windsurf --install-extension ./extension-performance-monitor-latest.vsix --force 2>/dev/null || true; \
	fi
	@if command -v cursor >/dev/null 2>&1; then \
		echo "Installing to Cursor..."; \
		cursor --install-extension ./extension-performance-monitor-latest.vsix --force 2>/dev/null || true; \
	fi
	@if command -v code-server >/dev/null 2>&1; then \
		echo "Installing to code-server..."; \
		code-server --install-extension ./extension-performance-monitor-latest.vsix --force 2>/dev/null || true; \
	fi
...
install-code-insiders: build
	@echo "Installing to VS Code Insiders..."
	code-insiders --install-extension ./extension-performance-monitor-latest.vsix --force

install-code: build
	@echo "Installing to VS Code..."
	code --install-extension ./extension-performance-monitor-latest.vsix --force

install-windsurf: build
	@echo "Installing to Windsurf..."
	windsurf --install-extension ./extension-performance-monitor-latest.vsix --force

install-cursor: build
	@echo "Installing to Cursor..."
	cursor --install-extension ./extension-performance-monitor-latest.vsix --force

install-code-server: build
	@echo "Installing to code-server..."
	code-server --install-extension ./extension-performance-monitor-latest.vsix --force
...
uninstall:
	...
	@if command -v code-insiders >/dev/null 2>&1; then \
		...
		code-insiders --uninstall-extension apertacodex.extension-performance-monitor 2>/dev/null || true; \
	fi
	@if command -v code >/dev/null 2>&1; then \
		echo "Uninstalling from VS Code..."; \
		code --uninstall-extension apertacodex.extension-performance-monitor 2>/dev/null || true; \
	fi
	@if command -v windsurf >/dev/null 2>&1; then \
		echo "Uninstalling from Windsurf..."; \
		windsurf --uninstall-extension apertacodex.extension-performance-monitor 2>/dev/null || true; \
	fi
	@if command -v cursor >/dev/null 2>&1; then \
		echo "Uninstalling from Cursor..."; \
		cursor --uninstall-extension apertacodex.extension-performance-monitor 2>/dev/null || true; \
	fi
	@if command -v code-server >/dev/null 2>&1; then \
		echo "Uninstalling from code-server..."; \
		code-server --uninstall-extension apertacodex.extension-performance-monitor 2>/dev/null || true; \
	fi
	@echo "Extension uninstallation completed"

# Uninstall from specific IDE
uninstall-code-insiders:
	code-insiders --uninstall-extension apertacodex.extension-performance-monitor

uninstall-code:
	code --uninstall-extension apertacodex.extension-performance-monitor

uninstall-windsurf:
	windsurf --uninstall-extension apertacodex.extension-performance-monitor

uninstall-cursor:
	cursor --uninstall-extension apertacodex.extension-performance-monitor

uninstall-code-server:
	code-server --uninstall-extension apertacodex.extension-performance-monitor
...
publish-openvsx:
	...
	npx ovsx publish ./extension-performance-monitor-latest.vsix -p $$OVSX_PAT
...
verify:
	...
	@for ext_dir in ~/.vscode/extensions/apertacodex.extension-performance-monitor-* ~/.vscode-insiders/extensions/apertacodex.extension-performance-monitor-*; do \
		if [ -d "$$ext_dir" ]; then \
